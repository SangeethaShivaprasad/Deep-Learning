	...
	def backward(self, error_tensor):
		...
		for image_idx in range(np.shape(error_tensor_with_stride)[0]):
			for channel_idx in range(self.convolution_shape[0]):
				for kernel_idx in range(self.num_kernels):
					...
					bias_gradient_image[image_idx, kernel_idx,:] = np.sum(error_tensor_with_stride[image_idx, kernel_idx, :])
		...
		for image_idx in range(np.shape(input_tensor_padded)[0]):
			...
			for kernel_idx in range(np.shape(error_tensor_with_stride)[1]):
				...
				for bias_image_idx in range(np.shape(input_tensor_padded)[0]):
					self.bias_gradient[kernel_idx] = np.sum(bias_gradient_image[bias_image_idx, kernel_idx,:])
		...
		self.input_kernel = self.input_kernel + self.gradient
